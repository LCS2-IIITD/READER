...
class PPOTrainer:
    ...
    def step(self, queries, masks, responses, scores, dac_label):
        """
        ...
        for _ in range(self.ppo_params['ppo_epochs']):
            ...
            for i in range(bs):
                ...
                train_stats = self.train_minibatch(logprobs[idx].unsqueeze(0), values[idx].unsqueeze(0),
                                                   ...
                                                   torch.cat([queries[idx],responses[idx]]).unsqueeze(0), dac_label[idx].unsqueeze(0))
    ...
    def batched_forward_pass(self, queries, responses):
        ...
        for i in range(int(bs/fbs)):
            ...
            with torch.no_grad():
                logits, d, v = self.model(input_ids).logits, self.model(input_ids).dac_pred , self.model(input_ids).value
    ...
    def train_minibatch(self, logprobs, values, rewards, query, response, model_input, dac_label):
        """Train one PPO minibatch"""
        dac_loss, lm_loss, loss_p, loss_v, train_stats  = self.loss(logprobs, values, rewards, query, response, model_input, dac_label)       
        loss = loss_p + loss_v  + 0.001*dac_loss  + lm_loss
        # print(loss_p, " : ", loss_v, " : ", lm_loss, " : ", dac_loss)
    ...
    def compute_rewards(self, scores, logprobs, ref_logprobs):
        ...
        for score, logprob, ref_logprob in zip(scores, logprobs, ref_logprobs):
            kl = F.relu(logprob - ref_logprob)
            non_score_reward = -self.kl_ctl.value * kl* 0.001
    ...
    def loss(self, old_logprobs, values, rewards, query, response, model_input, dac_label):
        ...
        logits, dac, vpred = self.model(model_input).logits, self.model(model_input).dac_pred , self.model(model_input).value
        logprob = logprobs_from_logits(logits[:,:-1,:], model_input[:, 1:])
        dac_loss = 0
        if dac_label is not None:
            # print(dac_label)
            # print(dac)
            criterion = nn.CrossEntropyLoss()
            dac_loss = criterion(target = dac_label, input = dac)
        ...
        diff = (logprob - old_logprobs)
        # if diff <1e-7:
        #     ratio = 1 + diff
        # else:
        # ratio = torch.exp(F.relu(diff) + 1e-18)
        ...
        lm_loss = self.model(input_ids = model_input, labels = model_input).loss
        ...
        stats = dict(
            loss=dict(lm = lm_loss, dac = dac_loss, policy=pg_loss, value=vf_loss, total=loss),
        ...
        return dac_loss, lm_loss, pg_loss, self.ppo_params['vf_coef'] * vf_loss, flatten_dict(stats)
